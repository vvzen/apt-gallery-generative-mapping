...
void ofApp::setup(){
    ...
    doorSeqSource = new DoorSeqSource();
    doorSeqSource->setup();
    piMapper.registerFboSource(doorSeqSource);

    doorLightsSource = new DoorLightsSource();
    doorLightsSource->setup();
    piMapper.registerFboSource(doorLightsSource);
...
void ofApp::update(){
    ...
    int numberOfSeconds = 3*60; // this kills it after 180 sec (change it to whatever you want MAX 180 seconds)
